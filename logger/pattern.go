...
// * ``%m``: The log message.
// * ``%c[{<maximumNumberOfElements>}]``: Holds the logging category. Normally this is the name of the logger or the service. If you do not specify ``maximumNumberOfElements`` the full name is displayed. If this is for example ``%c{2}`` and the name of the category is ``a.b.c`` then the output result is ``b.c``.
// * ``%F[{<maximumNumberOfPathElements>}]``: Holds the source file that logs this event. If you do not specify ``maximumNumberOfPathElements`` the full file name is displayed. If this is for example ``%F{2}`` and the file name is ``/a/b/c.go`` then the output result is ``b/c.go``.
// * ``%l`: Holds the source location of the log event.
// * ``%L`: Holds the line number where the log event was created.
// * ``%C[{<maximumNumberOfElements>}]``: Holds the source code package. If you do not specify ``maximumNumberOfElements`` the full name is displayed. If this is for example ``%C{2}`` and the name of the package is ``a.b.c`` then the output result is ``b.c``.
// * ``%M`: Holds the method name where the log event was created.
// * ``%p`: Holds the priority or better called log level.
// * ``%P[{<subFormatPattern>}]`: Stacktrace of the location where a problem was raised that caused this log message.
